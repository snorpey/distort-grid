...
export class ControlsUI {
	constructor() {
		...
		this.controlEls = Array.from(
			wrapperEl.querySelectorAll('[data-control-id]')
		);
	...
	stateUpdated(state) {
		Object.keys(state).forEach(key => {
			this.getInputEls(key).forEach(el => {
	...
	controlUpdated(input) {
		if (Array.isArray(input)) {
			...
			stateManager.update(
				this.getInputKey(inputEl.id),
				this.getInputValue(inputEl)
			);
		}
	}
	...
	getInputEls(id) {
		return this.controlEls.filter(
			el => el.getAttribute('data-control-id') === id
		);
	}
