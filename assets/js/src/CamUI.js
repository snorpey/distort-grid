...
export class CamUI {
	constructor() {
		...
		checkVideoInput().then(hasVideoInput => {
			if (hasVideoInput) {
				this.camButtonEl.removeAttribute('visibility');
				this.camButtonEl.addEventListener('click', () =>
					this.toggleCameraDisplay()
				);
				...
				this.shutterBtnEl.addEventListener('click', () =>
					this.takePicture()
				);
	...
	openCamera() {
		...
		requestVideoStream().then(stream => {
			this.stream = stream;
	...
	takePicture() {
		if (this.stream) {
			...
			eventBus.emit('image:src', imageSrc);
